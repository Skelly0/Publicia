    ...
    async def set_model(interaction: discord.Interaction, model: str):
        ...
        try:
            ...
            if success:
                ...
                # Create a description of all model strengths
                model_descriptions = [
                    f"**Gemini 2.5 Flash**: __RECOMMENDED__ - Best for prompt adherence, accurate citations, image viewing capabilities, and fast response times. Prone to hallucinating if asked about something not in it's supplied documents. Uses more search results ({bot.config.get_top_k_for_model('google/gemini-2.5-flash-preview')}) for broader context.",
                    ...
                    f"**Claude 3.5 Haiku**: __RECOMMENDED__ - A good balance between creativity and accuracy, and has image viewing capabilities. One of the best for longer roleplays. Uses a moderate number of search results ({bot.config.get_top_k_for_model('anthropic/claude-3.5-haiku')}) for balanced context.",
                    f"**Claude 3.5 Sonnet**: Advanced model similar to Claude 3.5 Haiku, the best model (admin only). Uses fewer search results ({bot.config.get_top_k_for_model('anthropic/claude-3.5-sonnet')}) to save money.",
