...
class ConversationManager:
    ...
    def get_conversation_messages(self, username: str, limit: int = 50) -> List[Dict]:
        ...
        for msg in messages:
            ...
            if channel:
                content = f"[Message was sent in Discord channel: {channel}] {content}"
            ...
            try:
                ...
                for doc, chunk, sim in relevant_docs:
                    if doc in googledoc_mapping:
                        ...
                        doc_contexts.append(f"From document with title: '{doc}' [Citation URL: {doc_url}] (similarity: {sim:.2f}):\n{chunk}")
                    else:
                ...
                if completion and completion.get('choices'):
                    ...
                    await interaction.followup.send("I apologize, but I'm having trouble generating a response right now, something snagged in my brain.")

            except Exception as e:
                logger.error(f"Error processing query: {e}")
                await interaction.followup.send("Blegh, my brain is struggling and an error has occurred.")
...
async def main():
    try:
        ...
        logger.info("awaiting commands from authorized imperial personnel...")
        bot = DiscordBot()
        async with bot:
